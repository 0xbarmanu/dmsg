/* $Id: miniupnpc.c,v 1.151 2018/02/22 15:07:16 nanard Exp $ */
...
static int SetRedirect(struct UPNPUrls * urls,
			       struct IGDdatas * data,
			       const char * iaddr,
			       const char * iport,
			       const char * eport,
...
int registerPort(char * ip, char * port, char * external_port, char* protocol, char * duration)
{
	struct UPNPDev * devlist = 0;
	...
	if( devlist )
	{
		...
		if( i )
		{
			...
			retcode = SetRedirect(&urls, &data,
						ip, port,
						external_port, protocol, duration, intClient, intPort);
